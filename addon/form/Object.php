...
class Form_Object extends Sabel_Object
{
  ...
  {
    $this->model->save($additionalValues);
  ...
  public function apply(array $values, array $allowCols = array())
  {
    if (empty($values)) return $this;
    
    if (empty($allowCols)) {
      $allowCols = $this->getAllowColumns();
    }
    ...
    foreach ($values as $key => $value) {
      if (strpos($key, self::NAME_SEPARATOR) === false) continue;
      list ($name, $colName) = explode(self::NAME_SEPARATOR, $key);
      ...
      if ($colName === "sbl_datetime") {
        foreach ($value as $key => $date) {
          ...
          if ($this->isDatetimeValuesCompleted($date)) {
            ...
            $model->__set($key, null);
          }
        }
      } elseif ($colName === "sbl_date") {
        foreach ($value as $key => $date) {
          if ($this->isDatetimeValuesCompleted($date, false)) {
            $model->__set($key, "{$date["year"]}-{$date["month"]}-{$date["day"]}");
          } else {
            $model->__set($key, null);
          }
        }
      } else {
        $model->__set($colName, $value);
      }
    }
  ...
  protected function isDatetimeValuesCompleted($values, $isDatetime = true)
  {
